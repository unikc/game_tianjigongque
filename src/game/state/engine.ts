...
export function resolveElimination(
  ...
  // ── 失败五：体力预警后仍强撑（猝然病倒）──────────────────────────
  // 条件：玩家在最终预警时选择了继续强撑（ignored_physician_final tag）
  // 且之后体力仍然没有恢复
  if (
    state.tags.includes("ignored_physician_final") &&
    state.stats.体力 <= 1 &&
    ...
    return {
      ...
      prose: [
        ...
        "",
        "这一章结束之前，你在回廊里晕倒了。",
        "不是戏文里那种体面的晕倒，是真的倒在地上，",
        "被两个不认识的宫人抬进了厢房。",
        "",
        "你没有办法用「我还有事情要做」来解释这件事，",
        "因为你已经没有办法说话了。",
        "",
        "三天后，你被移出了正式名册。",
        "太医在档案上写了一句话——你没有看见，也不会再有机会看见。",
      ].join("\n"),
    };
  }
  ...
  // ── 失败四：帝心耗尽（冷宫）──────────────────────────────────────
  // 条件：帝王宠爱 <= 5 且信任 <= 5，且连续 4 章无召见，且章节 >= 7
  // 有了「帝心冷却」副本作预警，这里是最终结果
  if (
    chapterNum >= 7 &&
    state.emperor.favor <= 5 &&
    state.emperor.trust <= 5 &&
