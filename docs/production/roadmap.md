...
| E06 | 自证预言 | P1 | `DONE` | L | E03、F01 | Narrative + Gameplay |
...
- 原因：E05背叛、E06预言、E04废后全部到位，第11-12章已有足够的道德重量。E09让玩家面对「赢了之后你是什么人」这个问题，需要在结局判定里从if-else链改为门槛制。
- 第一步：把 resolveEnding 的 if-else 链改为门槛制：每个结局都有必要条件+充分条件，没有兜底的 else。
- 验收：每个结局都有明确的进入条件；玩家可以事后理解自己为什么得到这个结局；至少一个结局需要背叛过人才能进入。
